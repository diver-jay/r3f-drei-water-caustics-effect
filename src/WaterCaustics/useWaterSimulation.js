...
export function useWaterSimulation(resolution = 256, enableAutoDrops = true) {
  ...
  const simulation = useMemo(() => {
    ...
    const textureA = new THREE.WebGLRenderTarget(resolution, resolution, options)
    const textureB = new THREE.WebGLRenderTarget(resolution, resolution, options)
  ...
  const addDrop = useCallback((x, y, radius = 0.03, strength = 0.3) => {
    const { scene, camera, mesh, dropMaterial, textureA, textureB } = simulation
  ...
  const update = () => {
    const { scene, camera, mesh, updateMaterial, normalMaterial, textureA, textureB } = simulation
    const source = simulation.currentTexture === 0 ? textureA : textureB
    const target = simulation.currentTexture === 0 ? textureB : textureA
  ...
  const getTexture = () => {
    ...
    return simulation.currentTexture === 0 ? textureA.texture : textureB.texture
  }
